...
long long count_size_c(union tree t){
	TreeKind tag = t.node.kind;
	if(tag == NODE) return sizeof(t) + count_size_c(*t.node.left) + count_size_c(*t.node.right);
	else{
		return sizeof(t);
	}
}
long long count_size_adt(Tree t){
	if_Node(t, l, r)
		return sizeof(struct _ADT_Tree_Node) + count_size_adt(l) + count_size_adt(r);
	else()
		return sizeof(struct _ADT_Tree_Leaf);
	end_if()
...
int add_tree_c(union tree t){
	...
	if(tag == NODE) return add_tree_c(*t.node.left) + add_tree_c(*t.node.right);
}
int add_tree_adt(Tree t){
	if_Leaf(t, val)
		return val;
	end_if()

	if_Node(t, l, r)
		return add_tree_adt(l) + add_tree_adt(r);
	end_if()
...
int main(){
	...
	FILE *fp;
	if((fp = fopen(".//result2", "a+")) == NULL){
		fprintf(stderr, "Cannot open file\n");
		exit(-1);
	}
	 ;

	long long sum_c = count_size_c(t);
	long long sum_adt = count_size_adt(adt_t);
